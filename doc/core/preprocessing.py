...
class FinancialTweetTokenizer:
    ...
    def _is_removable(self,token):
        if token.lower_ in self.stop_words:
            ...
            return True
        elif re.match("\$[a-zA-Z]+",token.text):
            return True
        elif "@" in token.text:
            return True
        elif token.text == " ":
    ...
    def __call__(self,text):
        ...
        tokens = [token.lower_ for token in tokens if not self._is_removable(token)]
...
if __name__=='__main__':
    ...
    logging.info("Saved the preprocessed file to"+blue+" './ouput/preprocessed_tweets.parquet'")